...
class DeepGCN(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, channels[0], 128, 128))
        HW = 224 // 4 * 224 // 4
    ...
    def forward(self, inputs):
        ...
        x22= self.pa2(x2)
        x3 = self.backbone[5:12](x2) 
        x33= self.pa3(x3)
        x4 = self.backbone[12:15](x3) 
        x44= self.pa4(x4)
        #x = x4
        #x = F.adaptive_avg_pool2d(x, 1)
        #x11: 32 128 128            x22: 32 64 64                    x33: 32 32 32               x44: 32 16 16
...
def pvig_b_224_gelu(pretrained=False, **kwargs):
    ...
    return model
# model = pvig_s_224_gelu(pretrained=True)
# pretrain_path = "/root/SourceCode/CropLand-CD-new-vig/pretrain/pvig_s_82.1.pth"
